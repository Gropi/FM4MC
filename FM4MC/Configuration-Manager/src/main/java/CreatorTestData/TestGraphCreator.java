...
public class TestGraphCreator {
    ...
    private IVertex optimizeVertex(IVertex vertex) {
        ...
        vertex.updateWeight(MeasurableValues.CPU.name(), 100);
        vertex.updateWeight(MeasurableValues.RAM.name(), 1500);
    ...
    private IVertex createVertex(String label, String serviceName, int applicationIndex, int stage, int approximation, int QoR) {
        var nextVertex = new Vertex(label, 31 * label.hashCode() + serviceName.hashCode(), serviceName);
    ...
    private Edge createEdge(IVertex start, IVertex end) {
        var nextEdge = new Edge(start, end, 31 * start.hashCode() + end.hashCode());
