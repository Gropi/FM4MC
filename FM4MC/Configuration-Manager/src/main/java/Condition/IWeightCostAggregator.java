...
public class IWeightCostAggregator {
    ...
    private IWeight stupidMergePathCosts(IWeight path1, IWeight path2) {
        ...
        for (var cost : costs1) {
            ...
            if(cost.getParameterName().equalsIgnoreCase(MeasurableValues.QoR.name())) {
                var costMultiplier = costs2.stream().filter(x -> x.getParameterName().equalsIgnoreCase(cost.getParameterName())).findFirst().orElse(new ParameterCost(1, cost.getParameterName()));
